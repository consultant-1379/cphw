...
int run_as_console()
{
    ...
    try
    {
    	...
    	CpTableObserver* observer = new CpTableObserver(architecture, mauType, changes);
    	//observer->init();
    	ACS_CS_API_SubscriptionMgr::getInstance()-> subscribeCpTableChanges(*observer);
		...
		ACS_CS_API_NS::CS_API_Result res = ACS_CS_API_SubscriptionMgr::getInstance()-> unsubscribeCpTableChanges(*observer);
		delete observer;
