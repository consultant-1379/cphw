...
string	IPAddressTable::s_iniPath = "/var/cpftp/apz/data/syscon/";
...
IPAddressTable::IPAddressTable(): /*m_addrStrTable(),*/ m_addrTable()
{
	...
	while(getline(file, line))
	{
		...
		do
		{
			...
			pos = m[4].str().find("169");
			if (pos == string::npos)
				break;

			ipAddr = m[4].str();
			ipAddr.replace(pos, 3, "170");

			//m_addr.insert(make_pair(pack32(cpId, side), m[4].str()));
			TRACE(("cpId <%d> side <%d> sol Ip1 <%s> sol Ip2 <%s>", cpId, side, m[4].str().c_str(), ipAddr.c_str()));
			//TRACE(("sol Ip2 <%s>", ipAddr.c_str()));

			m_addrStrTable.insert(make_pair(pack32(cpId, side), IpAddrPairT(m[4].str(), ipAddr)));
	...
	if (GepFunx::s_gepInfo)
	{
		GepFunx::s_gepInfo->getSolIpAddr(m_addrTable);

		int cpId, side;
		string solA, solB;
		for(map<int, IpAddrPairT>::iterator it = m_addrTable.begin(); it != m_addrTable.end(); ++it)
		{
			Funx::unpack32(it->first, cpId, side);
			solA = it->second.get<0>();
			solB = it->second.get<1>();

			TRACE(("cpId <%d> side <%d> solIpA <%s> solIpB <%s>", cpId, side, solA.c_str(), solB.c_str()));
			//TRACE(("solIpA <%s> solIpB <%s>", solA.c_str(), solB.c_str()));
		}
	}
