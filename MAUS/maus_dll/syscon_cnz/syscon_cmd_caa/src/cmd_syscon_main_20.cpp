...
void execute(int cpId, int side, int multiCpSystem, int)
{
	...
	while (true)
	{
		...
		replyCount = echo.replyCounter();
		if (replyCount < 9)
		{
			throw eNetworkUnstable;
		}
		index = echo.index();
		cout << "Ping is ok on interface <" << index << "> " << ipAddress[index] << endl;

		// Check Sol history
		// Sol already found error on this network
		if (solError[index] > 0)
		{
			// Sol also found error on the other network
			if (solError[index^1] > 0)
			{
				throw eBothSolServerDown;
			}
			// Try Sol on the other network
			index = index^1;
		}

		// Get a SOL Session on the other network and start it here
		usleep(35000);
		...
		solError[index] = terminal->getStatus();
		cout << "Connection time out on " << ipAddress[index] << endl;
