...
void unlock_maus(const string& sname, int si);
void unlock_maus(const string& sname);
...
int main(int argc, char** argv)
{
	...
	try {
		...
		// Unlocking service using cp anme
		int cpId = 0xffff;
		string sname;
		...
		else {	// Unlocking service using service name
			sname = s_serviceName;
		}

		// All parsing errors are found at this point in time
		if (s_si)
			unlock_maus(sname, s_si);
		else
			unlock_maus(sname);
	}
	catch (Cmd_Error_Code x)
	{
		...
		case eMandatoryOption:
			if (s_multiCpSystem)
			{
				cout << "Usage: mauunlock -cp cpName" << endl;
			}
			else
			{
				cout << "Usage: mauunlock" << endl;
			}
			cout << "Or:    mauunlock -n sname" << endl;
			break;
...
{
	static const string s_releaseOptions = "c:n:";
	...
	try
	{
		while ((c = getopt_long (argc, argv, cmdOpt.c_str(), long_options, 0)) != -1)
		{
			...
			case 'c':
				...
				if (strcmp(argv[optind - 1], "-cp") != 0 || cstr || siStr || optind == argc ||
						s_force || nStr)
					throw eSyntaxError;
...
void unlock_maus(const string& sname)
{
	...
	if (!si1_locked && !si2_locked)
		throw eAllInstancesUnlocked;

	string safsi = "SI-";
	string siStr = safsi + "1";
	if (si1_locked && unLockSi(dn1))
		throw InvalidSi(siStr, eUnlockingFailed);

	siStr = safsi + "2";
	if (si2_locked && unLockSi(dn2))
		throw InvalidSi(siStr, eUnlockingOneOK);
}
...
void unlock_maus(const string& sname, int si)
{
	...
	if (!si_locked)
		throw InvalidSi(siStr.c_str(), eInstanceUnlocked);
	if (unLockSi(dn))
		throw InvalidSi(siStr.c_str(), eUnlockingFailed);
}
