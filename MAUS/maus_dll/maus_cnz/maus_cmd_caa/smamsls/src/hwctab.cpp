...
void test(int fbn)
{
    ...
    if (hwcTable)
    {
       ...
       if (boardSearch)
       {
          ...
          unsigned short int systype, sysno, fbn, seqno, sysid;

     	cout << "systype" << " " << "sysno" << " " << "fbn"  << " " << "seqno" << endl;
         if (hwcTable->getBoardIds(boardList, boardSearch) == ACS_CS_API_NS::Result_Success)
          {
           	  for (unsigned int i = 0; i < boardList.size(); ++i)
           	  {
           		...
           		hwcTable->getSeqNo(seqno, boardList[i]);
           		hwcTable->getSysId(sysid, boardList[i]);

           		cout << systype << " " << sysno << " " << fbn  << " " << seqno << " " << sysid << endl;
           	  }
          }
          else
          {
