...
const string& TRACE_NAME = text_string[cMaus1CompName];
...
EventReporter EventReporter::s_instance(1);
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 1) {
        ...
        retVal = run_as_amf_service(procName, 1);
    }
    else if (argc < 3) {
    	string t(argv[1]);
    	...
    	if (t == NOSERVICE) {
            retVal = run_as_console(procName, 1);
        }
        else {
            help(procName);
            retVal = EXIT_FAILURE;
...
int run_as_amf_service(const string& name, int instance)
{
	...
	maus.start();
	TRACE_DEBUG(("main() %s returns", name.c_str())); //TR_HZ61131
...
int run_as_console(const string& name, int instance)
{
    ...
    //cout << "Check fd then press enter key" << endl;
    //cin.get();
