		...
		long eventNumber)
{
	...
	TRACE_DEBUG(("AEH Event, proc name <%s>, event number <%d>", procName.str().c_str(), eventNum));
	TRACE_DEBUG(("problemData <%s>", problemData));
	if (ret == ACS_AEH_error)
	{
                //TR_HZ61131: Corrected below traces
		TRACE_ERROR(("acs_aeh_EH_EvReport error, code: %d", m_evRep.getError()));
		TRACE_ERROR(("ACS error description: %s", m_evRep.getErrorText()));
	}
