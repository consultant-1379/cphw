...
public:
	...
	// Operation indicator; m_opInd
	// m_opInd == 0; install mibs as in the [cur] section of maus[12].conf
	// m_opInd == 1; install mibs as in the [section] specified in m_confSection
	// m_opInd == other values; reserved
	//
	// Configuration Section; m_confSection
	// m_confSection == 0; [new] section
	// m_confSection == 1; [cur] section
	// m_confSection == 2; [bak] section
	class InstallMibsMsg: public MsgHeader
	{
	public:
		static const PrimitiveT s_primitive;
		InstallMibsMsg(int opid, int config, int maj = VER_MAJOR, int min = VER_MINOR);

		int32_t	operationIndicator() const { return m_opInd; }
	...
	class InstallMibsRspMsg: public MsgHeader
	{
	public:
		static const PrimitiveT s_primitive;
		InstallMibsRspMsg(int32_t error, int maj = VER_MAJOR, int min = VER_MINOR):
